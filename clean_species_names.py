...
def clean_species_name(species_value, genus_value=None):
    ...
    Examples:
        ...
        "Propylea unclassified" → (None, None, "Propylea unclassified")
        "Coccotrypes nr" → (None, None, "Coccotrypes nr")
        "Coccotrypes nr cyperi" → (None, None, "Coccotrypes nr cyperi")
    """
    ...
    # Pattern 2: Uncertainty qualifiers - not a confirmed identification
    # Covers "aff."/"cf." (similar to / compare with) and "nr"/"nr." (near)
    if species_epithet.lower() in ['aff.', 'cf.', 'nr', 'nr.']:
        return None, None, species_str

    # Pattern 3: Non-taxonomic descriptors that are not valid species epithets
    # e.g. "unclassified", "unknown", "undetermined", "indet"
    _INVALID_EPITHETS = {
        'unclassified', 'unknown', 'undetermined', 'indet', 'indet.',
        'undescribed', 'unnamed',
    }
    if species_epithet.lower() in _INVALID_EPITHETS:
        return None, None, species_str

    # Pattern 4: Valid species epithet
...
def analyze_species_column(metadata_path='metadata.csv'):
    ...
    logger.info(f"  sp./nr/unclassified/etc (invalid): {results['sp_variants']} ({results['sp_variants']/results['total']*100:.1f}%)")
    ...
    logger.info("\nExamples of sp./nr/unclassified variants (will be set to NaN):")
...
def clean_metadata(input_path='metadata.csv', output_path='metadata_cleaned.csv'):
    ...
    logger.info(f"  Species invalidated (sp./nr/unclassified/etc): {invalidated_count}")
...
def main():
    ...
    output_file = Path('metadata_clean.csv')
